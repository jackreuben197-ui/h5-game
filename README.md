...
#### light-panel — стеклянная подложка светлой темы

Именованный паттерн подложки для панелей и модалок в белой теме. Миксин `light-panel`
лежит в `src/styles/_mixins.scss`, подключается как `@use '@/styles/mixins' as *;`.

Эталон — стекло панели уведомлений `.notification-panel`
(`src/components/BridgePanel/panels/BridgeNotificationPanel.vue`), значения перенесены один в один:

| Слой | Что даёт |
|------|----------|
| хост | обводка `rgba(242,242,242,0.04)`, радиус `clamp(28px, 10vw, 36.4px)`, `overflow: hidden`, свои тени сняты |
| `::after` | заливка `rgba(135,134,134,0.032)`, `backdrop-filter: blur(0.58px)`, внутренний светлый блик `rgba(242,242,242,0.7)` и тёмная виньетка `#000` |
| `::before` | градиентная рамка `0.4 → 0 → 0.5` через `mask-composite` |

Параметры: `$fill`, `$blur`, `$radius` — значения по умолчанию как у эталона.

Заливка почти прозрачная намеренно: тон подложки задаёт не она, а то, что за модалкой,
приглушённое оверлеем диалога. Плотная заливка (пробовали `rgba(255,255,255,0.8)`) убивает
стекло и даёт плоское серое пятно.
...
Для модалок на `GameDialog` есть готовая обёртка `light-panel-dialog`: светлая тема красит
`.game-dialog__card` в плоский белый (`GameDialog.vue`, блок `:root[data-theme='light']`), и это
правило перебивает `light-panel` — миксин возвращает стекло и светлый текст поверх него
(`__title`, `__body`, `__cancel-btn`). Подключать с селектора самой модалки, тогда специфичности
хватает:
...
Применён в `.login-dialog`, `.club-notice-dialog`, `.template-dialog`, `.logout-dialog`. Панель
уведомлений (`.notification-dialog`) стекло приносит сама, поэтому там карточка-хост держится
прозрачной в `GlobalBridgePanelHost.vue`.
...
Миксин работает только внутри селектора: на верхнем уровне `&` пуст, и Sass молча выдаёт
`html[data-theme='light'] & .card` — невалидный селектор, который браузер отбрасывает целиком.
Такой блок выглядит написанным, но не применяется; для верхнего уровня писать
`:root[data-theme='light'] { ... }` руками.
