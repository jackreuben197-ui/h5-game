import type { MttFormState } from './formState'
import { defaultMttFormState } from './formState'
 ...
 */
export function buildRoomConfigPayload(formState: MttFormState): Record<string, unknown> {
  // ── 随机前注 ────────────────────────────────────────────────────
  ...
  return {
    ...
    call_time: formState.call_time,
    call_time_winline:
      formState.call_time === 1 ? parseInt(formState.call_time_winline || '0') || 0 : 0,
    call_time_count:
      formState.call_time === 1 ? parseInt(formState.call_time_count || '0') || 0 : 0,
...
// ─────────────────────────────────────────────────────────────────────────────
// 反向转换：服务端 room_config → MttFormState
...
/**
 * 将服务端 room_config 数据还原为 MttFormState。
 * 规则与 buildRoomConfigPayload 完全对称，参考 Unity FillAllUIByRoomConfig 系列方法。
 */
export function parseRoomConfigToFormState(c: Record<string, unknown>): MttFormState {
  const base = { ...defaultMttFormState }
